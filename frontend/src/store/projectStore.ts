...
interface ProjectsState {
  ...
  // Actions
  fetchProjects: (page?: number, limit?: number, forceRefresh?: boolean) => Promise<void>;
...
export const useProjectsStore = create<ProjectsState>()(
  persist(
    (set, get) => ({
      ...
      fetchProjects: async (page = 1, limit = 10, forceRefresh = false) => {
        const { projects, lastFetched } = get();
        const now = Date.now();

        // Si hay cache válido y no se fuerza refresh, usar cache
        if (!forceRefresh && projects.length > 0 && lastFetched && (now - lastFetched) < CACHE_TTL) {
          return; // Usar datos cacheados, no hacer request
        }
